...
- `npm run debug:timeline -- --latest` reads the newest opt-in provider debug log and renders a timeline. It does not enable logging itself (`package.json`, `scripts/debug-log-timeline.mjs`).
...
`proto/agent_pb.ts` identifies itself as generated by `protoc-gen-es` 2.10.2 from `agent.proto` and disables lint/type checking internally. No schema source or regeneration command is tracked. Do not hand-edit it. Regeneration procedure: **Unverified** (`proto/agent_pb.ts`, `package.json`).
